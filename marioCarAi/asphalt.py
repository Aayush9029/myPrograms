...
def process_img(original_image):
    ...
    # vertices = np.array(
    # ([0, 150], [100, 80], [300, 80], [400, 130], [400, 0], [0, 0]))
    # processed_img=cv2.GaussianBlur(processed_img, (3, 3), 0)
    # processed_img = roi(processed_img, [vertices])
    lines = cv2.HoughLinesP(processed_img, 1, np.pi / 180, 80, np.array([]),
                            25, 8)
    m1, m2 = 0, 0
    try:
        l1, l2, m1, m2 = draw_lanes(original_image, lines)
        cv2.line(original_image, (l1[0], l1[1]), (l1[2], l1[3]), [0, 255, 220],
                 5)
...
def main():
    with mss.mss() as sct:
        monitor = {"top": 135, "left": 50, "width": 700, "height": 370}
        while 1:
            screen = np.array(sct.grab(monitor))
            screen = cv2.resize(screen, (int(
                monitor["width"] / scale), int(monitor["height"] / scale)))
